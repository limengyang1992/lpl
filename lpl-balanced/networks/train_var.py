...
parser.add_argument('--pgd_nums', default=10, type=int,
                    help='total number of layers (have to be explicitly given!)')

parser.add_argument('--pgd_alpha', default=0.001, type=float,
                    help='total number of layers (have to be explicitly given!)')
...
class CATLoss(nn.Module):
    ...
    def compute_steps(self, logit,y):
        with torch.no_grad():
            ...
            split = torch.sum(torch.where(sub<0,1,0)).detach().cpu().numpy()
            class_step = get_step(split,classes_num=self.num_classes,step_size=self.pgd_nums,classes_freq=sort_values)
